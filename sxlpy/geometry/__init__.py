...
class MetricTensor:
	...
	def __init__(self, values: list[list[Expr]], cs: CoordinateSystem) -> None:
		...
		self.conn_ddd = [[[None for _ in range(dim(self))] for _ in range(dim(self))] for _ in range(dim(self))]
		self.conn_udd = [[[None for _ in range(dim(self))] for _ in range(dim(self))] for _ in range(dim(self))]
		...
		with Progress("Computing covariant connection coefficients", total) as pb:
			for i in range(d):
				for j, k in symmetric_with_diag(d):
					A = (diff(self.metric_co(i, j), self.coords.x(k)) + diff(self.metric_co(i, k), self.coords.x(j)) - diff(self.metric_co(k, j), self.coords.x(i)))/2
					self.conn_ddd[i][j][k] = A
					self.conn_ddd[i][k][j] = self.conn_ddd[i][j][k]
					pb.done()
		with Progress("Computing mixed-index connection coefficients", total) as pb:
			for i in range(d):
				for j, k in symmetric_with_diag(d):
					self.conn_udd[i][j][k] = sum(self.metric_contra(i, l) * self.conn_ddd[l][j][k] for l in range(d))
					pb.done()
		return None
